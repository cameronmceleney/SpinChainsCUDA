...
//  Dtype               Member Name                                Variable docstring
    ...
    // Description missing
    __device__ double              EffectiveFieldX (const int& site, const int& layer,
                                         const double& mxLHS, const double& mxMID, const double& mxRHS,
                                         const double& dipoleTerm, const double& current_time);

    // Description missing
    __device__ double              EffectiveFieldY (const int& site, const int& layer,
                                         const double& myLHS, const double& myMID, const double& myRHS,
                                         const double& dipoleTerm);

    // Description missing
    __device__ double              EffectiveFieldZ (const int& site, const int& layer,
    ...
    __device__ double              MagneticMomentXMulti (const int& spin, const int& layer,
    ...
    __device__ double              MagneticMomentYMulti (const int& spin, const int& layer,
                                         const double& mxMID, const double& myMID, const double& mzMID,
                                         const double& hxMID, const double& hyMID, const double& hzMID);

    // Full version of magnetic moment z-component function for multi-layered systems
    __device__ double              MagneticMomentZMulti (const int& spin, const int& layer,
                                         const double& mxMID, const double& myMID, const double& mzMID,
                                         const double& hxMID, const double& hyMID, const double& hzMID);
    ...
    // Description missing
    double* flattenNestedVector(const std::vector<std::vector<double>>& nestedVector);
     ...
     * ################################################################################################################
     */
    void RK2Stage1CUDA(int layer, double timeStep, double* m0NestFlattened, int m0FlatDim);
    void RK2Stage2CUDA(int layer, double timeStep, double* m1NestFlattened, int m1FlatDim);
